...
def plot_fgmax_grid(outdir,plotdir):
    ...
    clines_zeta = [-2.0, -1.0, -0.75, -0.5, -0.4, -0.3, -0.2, -0.1, -0.01, -0.001]
    #colors = geoplot.discrete_cmap_1(clines_zeta)
    colors = geoplot.discrete_cmap_2(clines_zeta)
    plt.figure(1)
    plt.clf()
    #zeta = numpy.where(fg.B>0, fg.h, fg.h+fg.B)   # surface elevation in ocean
    print(numpy.min(fg.h), numpy.max(fg.h))
    ...
    plt.title("Maximum negative tsunami height")
    plt.tight_layout()

    if not os.path.isdir(plotdir): 
        os.mkdir(plotdir)
    fname = os.path.join(plotdir, "amplitude_negative_Fra.png")
